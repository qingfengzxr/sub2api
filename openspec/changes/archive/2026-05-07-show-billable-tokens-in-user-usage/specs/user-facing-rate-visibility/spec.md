...
系统 MUST 在 C 端普通用户前端隐藏所有直接倍率展示，包括 `x倍`、`倍率`、`rate multiplier`、标准/实际双口径消费、原始/计费双口径消费和等价内部计费系数文案。系统 MAY 在导出中返回可用于账单校验的 raw token、billable token 和 `billing_token_multiplier` 字段。系统 SHALL 在 Usage 页面 token tooltip 中直接展示 billable token 明细，而不是将 raw usage 与 billable usage 渲染成两个并列口径。
...
- **THEN** 前端不得把 `billing_token_multiplier` 渲染为 group/account 内部倍率 badge 或内部运营策略说明
