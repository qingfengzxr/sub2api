...
系统 MUST 在 C 端普通用户前端隐藏所有直接倍率展示，包括 `x倍`、`倍率`、`rate multiplier`、标准/实际双口径消费、原始/计费双口径消费和等价内部计费系数文案。系统 MAY 在用量明细或导出中返回和展示可用于账单校验的 billable token 字段，但不得将其渲染为内部 group/account 倍率配置或旧的标准/实际双口径费用解释。
