...
系统 MUST 在 C 端普通用户前端隐藏所有直接倍率展示，包括 `x倍`、`倍率`、`rate multiplier`、标准/实际双口径消费、原始/计费双口径消费、raw/billable 双口径 token tooltip 和等价内部计费系数文案。系统 SHALL 在普通用户 Usage 页面 token tooltip 中直接展示最终 billable token 明细，而不是将 raw usage 与 billable usage 渲染成两个并列口径。系统 SHALL 在普通用户 Usage 页面费用 tooltip 或成本明细中展示基础标准单价，不得将 `billing_token_multiplier`、group/user 金额倍率、service tier、长上下文加价、图片倍率或其他内部放大倍率折算进输入/输出/缓存/图片等单价展示。系统 SHALL 在普通用户 Usage 页面 `TOKEN` 表格列中直接展示最终 billable token 数值，并确保该列与 token tooltip 的计费口径一致。系统 SHALL 在普通用户 Usage CSV 导出中使用与页面一致的 billable-first token 口径，并且不得导出 `Billable...` 列、`Billing...` 列、`billing_token_multiplier` 或等价内部计费策略字段。管理员后台不受该 C 端隐藏规则限制。
...
#### Scenario: 用户查看单条输入输出单价
- **WHEN** 普通用户在使用记录表格查看单条费用 tooltip 或成本明细
- **THEN** 输入、输出、缓存读取、缓存写入、图片或按次等单价展示必须使用该记录计费模型的基础标准单价
- **THEN** 单价展示不得通过 `input_cost / input_tokens`、`output_cost / output_tokens` 或其他最终成本拆分字段反推出倍率放大后的派生单价
- **THEN** 单价展示不得包含 `billing_token_multiplier`、group/user 金额倍率、service tier、长上下文加价、图片倍率或其他内部放大倍率影响
- **THEN** 如果系统无法确定某个基础标准单价，前端必须显示空态占位，不得回退展示放大后的派生单价
