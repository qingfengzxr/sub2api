...
func init() {
	...
	// userDescOverdraftLimit is the schema descriptor for overdraft_limit field.
	userDescOverdraftLimit := userFields[4].Descriptor()
	// user.DefaultOverdraftLimit holds the default value on creation for the overdraft_limit field.
	user.DefaultOverdraftLimit = userDescOverdraftLimit.Default.(float64)
	// user.OverdraftLimitValidator is a validator for the "overdraft_limit" field. It is called by the builders before save.
	user.OverdraftLimitValidator = userDescOverdraftLimit.Validators[0].(func(float64) error)
