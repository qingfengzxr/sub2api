...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	...
	// 计算费用
	cost := s.calculateRecordUsageCost(ctx, result, apiKey, billingModel, multiplier, imageMultiplier, billableUsage, pricingAt)
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedResponseModelPricing(ctx, responseModel, apiKey); identified {
			responseCost := s.calculateRecordUsageCost(ctx, result, apiKey, responseModel, multiplier, imageMultiplier, billableUsage, pricingAt)
...
func (s *GatewayService) calculateRecordUsageCost(
	...
	if result.ImageCount > 0 {
		if resolved := s.resolveChannelPricing(ctx, billingModel, apiKey); resolved != nil && resolved.Mode == BillingModeToken {
			return s.calculateTokenCost(ctx, result, apiKey, billingModel, multiplier, billableUsage, pricingAt)
		}
	...
	tokenCost := s.calculateTokenCost(ctx, result, apiKey, billingModel, multiplier, billableUsage, pricingAt)
