	...
	for _, applyLongContext := range []bool{false, true} {
		for _, serviceTier := range []string{"", "priority", "flex"} {
			for _, rateMultiplier := range []float64{-1, 0, 0.8} {
				want, err := bs.calculateCostInternalWithPolicy("gpt-5.4", tokens, rateMultiplier, serviceTier, nil, LongContextPricingPolicy{Enabled: applyLongContext, ThresholdTokens: DefaultLongContextPricingThresholdTokens})
				require.NoError(t, err)
