...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	// 平台策略、分组开关和可选的账号开关必须同时允许。
	longContextPolicy := LongContextPricingPolicy{}
	if len(resolved.Intervals) == 0 {
		longContextPolicy = input.LongContextPricing
		longContextPolicy.Enabled = longContextPolicy.Enabled && resolved.longContextPricingEnabled
		if input.LongContextBillingEnabled != nil {
			longContextPolicy.Enabled = longContextPolicy.Enabled && *input.LongContextBillingEnabled
		}
	...
	return s.computeTokenBreakdown(pricing, input.Tokens, input.RateMultiplier, input.ServiceTier, longContextPolicy), nil
}
