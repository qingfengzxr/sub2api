...
const (
	DefaultLongContextPricingThresholdTokens = 272000
	openAIGPT54LongContextInputThreshold     = DefaultLongContextPricingThresholdTokens
...
type LongContextPricingPolicy struct {
	Enabled         bool
	ThresholdTokens int
}

func normalizeLongContextPricingPolicy(policy LongContextPricingPolicy) LongContextPricingPolicy {
	if !policy.Enabled || policy.ThresholdTokens <= 0 {
...
type CostInput struct {
	...
	LongContextPricing        LongContextPricingPolicy // 平台级长上下文加价策略
...
func (s *BillingService) CalculateCostUnified(input CostInput) (*CostBreakdown, error) {
	if input.Resolver == nil {
		// 无 Resolver，回退到旧路径
		policy := input.LongContextPricing
		if input.LongContextBillingEnabled != nil {
			policy.Enabled = *input.LongContextBillingEnabled
			if policy.Enabled && policy.ThresholdTokens <= 0 {
				policy.ThresholdTokens = DefaultLongContextPricingThresholdTokens
			}
		}
		return s.calculateCostInternal(input.Model, input.Tokens, input.RateMultiplier, input.ServiceTier, nil, policy)
	}
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	// 分组开关与可选的账号开关任一开启即可；平台策略可覆盖官方阈值。
	longContextPolicy := LongContextPricingPolicy{}
	if len(resolved.Intervals) == 0 {
		longContextPolicy = input.LongContextPricing
		if !longContextPolicy.Enabled && pricing.LongContextInputThreshold > 0 {
			longContextPolicy = LongContextPricingPolicy{Enabled: true, ThresholdTokens: pricing.LongContextInputThreshold}
		}
		accountEnabled := input.LongContextBillingEnabled != nil && *input.LongContextBillingEnabled
		longContextPolicy.Enabled = longContextPolicy.Enabled && (resolved.longContextPricingEnabled || accountEnabled)
		if longContextPolicy.Enabled && longContextPolicy.ThresholdTokens <= 0 {
			longContextPolicy.ThresholdTokens = pricing.LongContextInputThreshold
		}
	}

	breakdown := s.computeTokenBreakdown(pricing, input.Tokens, input.RateMultiplier, input.ServiceTier, longContextPolicy)
...
func (s *BillingService) computeTokenBreakdown(
	...
	longContextPricingEligible := s.shouldApplySessionLongContextPricing(tokens, pricing, longContextPolicy)
	var baselineCost *CostBreakdown
	if longContextPricingEligible {
		baselineCost = s.computeTokenBreakdown(pricing, tokens, rateMultiplier, serviceTier, LongContextPricingPolicy{})
...
func (s *BillingService) CalculateCost(model string, tokens UsageTokens, rateMultiplier float64) (*CostBreakdown, error) {
	return s.calculateCostInternal(model, tokens, rateMultiplier, "", nil, LongContextPricingPolicy{})
}
...
func (s *BillingService) CalculateCostWithServiceTier(model string, tokens UsageTokens, rateMultiplier float64, serviceTier string) (*CostBreakdown, error) {
	return s.calculateCostInternal(model, tokens, rateMultiplier, serviceTier, nil, LongContextPricingPolicy{})
}

func (s *BillingService) CalculateCostWithServiceTierAndLongContextPolicy(model string, tokens UsageTokens, rateMultiplier float64, serviceTier string, policy LongContextPricingPolicy) (*CostBreakdown, error) {
	return s.calculateCostInternal(model, tokens, rateMultiplier, serviceTier, nil, policy)
}

func (s *BillingService) calculateCostInternal(model string, tokens UsageTokens, rateMultiplier float64, serviceTier string, channelPricing *ChannelModelPricing, policy LongContextPricingPolicy) (*CostBreakdown, error) {
	if !policy.Enabled && policy.ThresholdTokens == 0 {
		policy.Enabled = true
	}
	return s.calculateCostInternalWithPolicy(model, tokens, rateMultiplier, serviceTier, channelPricing, policy)
}
...
func (s *BillingService) calculateCostWithServiceTierPolicy(
	...
	return s.calculateCostInternalWithPolicy(model, tokens, rateMultiplier, serviceTier, nil, LongContextPricingPolicy{
		Enabled:         longContextBillingEnabled,
		ThresholdTokens: DefaultLongContextPricingThresholdTokens,
	})
}
...
func (s *BillingService) calculateCostInternalWithPolicy(
	...
	policy LongContextPricingPolicy,
) (*CostBreakdown, error) {
	...
	if policy.Enabled && policy.ThresholdTokens <= 0 {
		policy.ThresholdTokens = pricing.LongContextInputThreshold
	}

	return s.computeTokenBreakdown(pricing, tokens, rateMultiplier, serviceTier, policy), nil
}
...
func (s *BillingService) shouldApplySessionLongContextPricing(tokens UsageTokens, pricing *ModelPricing, policy LongContextPricingPolicy) bool {
	policy = normalizeLongContextPricingPolicy(policy)
	if pricing == nil || !policy.Enabled || policy.ThresholdTokens <= 0 {
		return false
	}
	...
	if pricing.LongContextThresholdInclusive {
		return totalInputTokens >= policy.ThresholdTokens
	}
	return totalInputTokens > policy.ThresholdTokens
}
