...
func TestComputeTokenBreakdown_GptImage2ImageEditIssue4386(t *testing.T) {
	...
	cost := svc.computeTokenBreakdown(pricing, tokens, 1.0, "", LongContextPricingPolicy{})
...
func TestNewModelPricingChannelOverridesAndFamilyIsolation(t *testing.T) {
	...
	for _, model := range []string{"gpt-6-sol", "gpt-6-luna", "claude-opus-5-5"} {
		t.Run(model, func(t *testing.T) {
			...
			require.NoError(t, err)
			cost := svc.computeTokenBreakdown(prices, UsageTokens{InputTokens: 300000, CacheReadTokens: 1000, CacheCreationTokens: 1000, OutputTokens: 1000}, 1, "priority", LongContextPricingPolicy{Enabled: true, ThresholdTokens: prices.LongContextInputThreshold})
