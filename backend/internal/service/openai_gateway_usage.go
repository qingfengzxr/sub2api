...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	billableUsage := BuildBillableUsage(tokens, s.billingTokenPolicy(ctx))
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedOpenAIResponsePricing(ctx, responseModel, apiKey); identified {
			...
			responseCost, responseErr := s.calculateOpenAIRecordUsageCost(
				...
				videoMultiplier, baseMultiplier, billableUsage, serviceTier, longContextBillingGate, pricingAt,
			)
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	if isGrokVideoUsageResult(result, billingModels) {
		if resolved := s.resolveOpenAIChannelPricing(ctx, billingModel, apiKey); resolved == nil || resolved.Mode != BillingModeToken {
			return s.calculateOpenAIVideoCost(ctx, billingModel, apiKey, result, videoMultiplier, billableUsage), nil
		}
	}
	...
	if result != nil && result.ImageCount > 0 {
		...
		if resolved := s.resolveOpenAIChannelPricing(ctx, billingModel, apiKey); resolved == nil || resolved.Mode != BillingModeToken {
			return s.calculateOpenAIImageCost(ctx, billingModel, apiKey, result, imageMultiplier, billableUsage), nil
		}
	}
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageTokenCost(
	...
	longContextPolicy.Enabled = longContextBillingGate == nil || *longContextBillingGate
	if s.resolver == nil && longContextPolicy.Enabled && longContextPolicy.ThresholdTokens <= 0 {
		longContextPolicy.ThresholdTokens = DefaultLongContextPricingThresholdTokens
	}
...
func (s *OpenAIGatewayService) billingTokenPolicy(ctx context.Context) BillingTokenPolicy {
