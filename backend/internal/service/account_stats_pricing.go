...
func tryModelFilePricing(billingService *BillingService, model string, tokens UsageTokens) *float64 {
	...
	if billingService.shouldApplySessionLongContextPricing(tokens, pricing, longContextPolicy) {
		breakdown, err := billingService.CalculateCostWithServiceTierAndLongContextPolicy(model, tokens, 1, "", longContextPolicy)
