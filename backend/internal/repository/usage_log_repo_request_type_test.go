...
func TestPrepareUsageLogInsert_PersistsNativeCompactionV2WithoutChangingRequestType(t *testing.T) {
	...
	require.Equal(t, int16(service.RequestTypeStream), prepared.args[40])
...
func TestPrepareUsageLogInsert_PersistsImageSizeMetadata(t *testing.T) {
	...
	require.Equal(t, sql.NullString{String: imageSize, Valid: true}, prepared.args[48])
	require.Equal(t, sql.NullString{String: inputSize, Valid: true}, prepared.args[49])
	require.Equal(t, sql.NullString{String: outputSize, Valid: true}, prepared.args[50])
	require.Equal(t, sql.NullString{String: source, Valid: true}, prepared.args[51])
	breakdownJSON, ok := prepared.args[52].(string)
	require.True(t, ok)
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			0, 0, 0, 0, 0, 0, 0, 0, 0, // billable usage fields
			sql.NullFloat64{Valid: true, Float64: 1},
