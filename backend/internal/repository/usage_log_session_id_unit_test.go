...
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 72, "arg-type table must include billable usage and upstream request ID fields")
...
func TestPrepareUsageLogInsert_RequestedReasoningEffortArgWiring(t *testing.T) {
	...
	require.Len(t, prepared.args, len(usageLogInsertArgTypes))
	require.Equal(t, "text", usageLogInsertArgTypes[58], "requested_reasoning_effort must follow reasoning_effort")
	require.Equal(t, "text", usageLogInsertArgTypes[57], "reasoning_effort arg type must stay text")

	forwardedArg, ok := prepared.args[57].(sql.NullString)
	require.True(t, ok)
	require.True(t, forwardedArg.Valid)
	require.Equal(t, forwarded, forwardedArg.String)

	requestedArg, ok := prepared.args[58].(sql.NullString)
	require.True(t, ok)
