...
func usageTokenSQLFor(alias string, billable bool) usageTokenSQL {
	...
	column := func(name string) string {
		...
		return "COALESCE(NULLIF(" + alias + "billable_" + name + ", 0), " + raw + ")"
	}
	return usageTokenSQL{
		input:         column("input_tokens"),
		output:        column("output_tokens"),
		cacheCreation: column("cache_creation_tokens"),
		cacheRead:     column("cache_read_tokens"),
	}
}
