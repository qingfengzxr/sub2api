...
describe('user UsageView', () => {
  ...
  it('exports csv with current filters and without admin-only fields', async () => {
    ...
    expect(csvContent).toContain('Token,10143,253,695680,10,0,1,0.09288300')
    const [headerLine, dataLine] = csvContent.split('\n')
    const headers = headerLine.split(',')
    const values = dataLine.split(',')
    expect(values).toHaveLength(headers.length)
    expect(values[headers.indexOf('Rate Multiplier')]).toBe('1')
    expect(values[headers.indexOf('Billed Cost')]).toBe('0.09288300')
    expect(values[headers.indexOf('Original Cost')]).toBe('0.09288300')
