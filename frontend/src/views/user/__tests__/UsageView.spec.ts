...
describe('user UsageView', () => {
  ...
  it('exports csv with current filters and without admin-only fields', async () => {
    ...
    expect(csvContent.slice(1)).toBe([
      'Time,API Key Name,Model,Reasoning Effort,Inbound Endpoint,IP Address,Type,Billing Mode,Input Tokens,Output Tokens,Cache Read Tokens,Cache Creation Tokens,Image Output Tokens,Billed Cost,First Token (ms),Duration (ms)',
      '2026-03-08T00:00:00Z,demo-key,gpt-5.4,"\'-",,203.0.113.10,Sync,Token,10143,253,695680,10,0,0.09288300,12,345',
    ].join('\n'))
    ...
    expect(headers).not.toContain('Rate Multiplier')
    expect(headers).not.toContain('Original Cost')
    expect(values[headers.indexOf('Input Tokens')]).toBe('10143')
    expect(values[headers.indexOf('Output Tokens')]).toBe('253')
    expect(values[headers.indexOf('Cache Read Tokens')]).toBe('695680')
    expect(values[headers.indexOf('Cache Creation Tokens')]).toBe('10')
    expect(values[headers.indexOf('Image Output Tokens')]).toBe('0')
    expect(values[headers.indexOf('Billed Cost')]).toBe('0.09288300')
    expect(csvContent).not.toContain('4057,101,278272,4')
    expect(csvContent).not.toContain('2.5')
