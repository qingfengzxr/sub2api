    ...
    <div class="space-y-6">
      ...
      <template v-if="activeTab === 'usage'">
        <UsageTable
          ...
          :show-billing-audit-details="false"
...
const exportTokenBreakdown = (log: UsageLog) => ({
  inputTokens: billableOrRaw(log, 'billable_input_tokens', 'input_tokens'),
  outputTokens: billableOrRaw(log, 'billable_output_tokens', 'output_tokens'),
  cacheReadTokens: billableOrRaw(log, 'billable_cache_read_tokens', 'cache_read_tokens'),
  cacheCreationTokens: billableOrRaw(log, 'billable_cache_creation_tokens', 'cache_creation_tokens'),
  imageOutputTokens: billableOrRaw(log, 'billable_image_output_tokens', 'image_output_tokens'),
})
...
const exportToCSV = async () => {
  ...
  try {
    ...
    const rows = allLogs.map((log) => {
      const tokens = exportTokenBreakdown(log)
      return [
        log.created_at,
        log.api_key?.name || '',
        log.model,
