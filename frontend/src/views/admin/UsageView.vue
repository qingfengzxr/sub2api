...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.billableInputTokens'), t('usage.billableOutputTokens'),
      t('usage.billableCacheReadTokens'), t('usage.billableCacheCreationTokens'),
      t('usage.billableImageOutputTokens'), t('usage.billingTokenMultiplier'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.billable_input_tokens || log.input_tokens || 0,
        log.billable_output_tokens || log.output_tokens || 0,
        log.billable_cache_read_tokens || log.cache_read_tokens || 0,
        log.billable_cache_creation_tokens || log.cache_creation_tokens || 0,
        log.billable_image_output_tokens || log.image_output_tokens || 0,
        log.billing_token_multiplier || 1,
